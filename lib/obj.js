...
(function(JSUS) {
    ...
    OBJ.split = (function() {
        var splitValue;
        var model, level, makeClone;

        makeClone = function(value, key, out, curLevel, posAsKey) {
            var copy;
            copy = JSUS.clone(model);

            switch(curLevel.length) {
            case 0:
                copy[key] = JSUS.clone(value);
                break;
            case 1:
                copy[key][curLevel[0]] = JSUS.clone(value);
                break;
            case 2:
                copy[key][curLevel[0]] = {};
                copy[key][curLevel[0]][curLevel[1]] = JSUS.clone(value);
                break;
            default:
                i = 0, len = curLevel.length-2;
                tmp = copy[key];
                for ( ; ++i < len ; ) {
                    tmp[curLevel[i]] = {};
                    tmp = tmp[curLevel[i]];
                }
                tmp[curLevel.length-1] = JSUS.clone(value);
            }

            out.push(copy);
            return;
            copy = JSUS.clone(model);
            if (!posAsKey && JSUS.isArray(value)) {
                copy[key] = JSUS.clone(value[i]);
            }
            else {
                copy[key][i] = JSUS.clone(value[i]);
            }
            out.push(copy);
        ...
        splitValue = function(value, key, out, curLevel, posAsKey) {
            var i, len, tmp, copy, recursive;

            // level == 0 means no limit.
            if (level && (curLevel.length >= level)) {
                makeClone(value, key, out, curLevel, posAsKey);
            }
            else {
                for (i in value) {
                    if (value.hasOwnProperty(i)) {
                        if ('object' === typeof value[i] &&
                            (level && ((curLevel.length+1) <= level))) {

                            splitValue(value[i], key, out,
                                       curLevel.concat(i),
                                       posAsKey);
                        }
                        else {
                            makeClone(value[i], key, out,
                                      curLevel.concat(i), posAsKey);
                        }
                    }
                }
            }
        ...
        return function(o, key, l, positionAsKey) {
            ...
            splitValue(o[key], [key], out, [], positionAsKey);
            // Cleanup.
            model = null;
            level = null;
            // Return.
