...
(function(JSUS) {
    ...
    ARRAY.inArray = function(needle, haystack) {
        return indexOfAll('inArray', needle, haystack, 'first') !== -1;
    };
    ...
    ARRAY.indexOf = function(needle, haystack) {
        return indexOf('indexOf', needle, haystack);
    };
     ...
     * @see ARRAY.indexOf
     */
    function indexOf(method, needle, haystack) {
        var i, len;
        ...
        if ('number' === typeof needle && isNaN(needle)) {
            ...
            for (i = 0; i < len; i++) {
                if (JSUS.equals.call(this, needle, haystack[i])) {
                    return i;
                }
            }
        }
     ...
     * @see ARRAY.indexOf
     */
    function indexOfAll(method, needle, haystack, mod, expectedN) {
        var i, len, idx, out;
        if (!ARRAY.isArray(haystack)) {
            throw new TypeError('JSUS.' + method + ': haystack must be ' +
                                'array. Found: ' + haystack);
