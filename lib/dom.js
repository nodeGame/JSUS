...
(function() {
    ...
    var isArray, randomInt, randomString, keys, size, sample, isInt, mixin;
    if ('undefined' !== typeof JSUS) {
        isArray = JSUS.isArray;
        randomInt = JSUS.randomInt;
        randomString = JSUS.randomString;
        keys = JSUS.keys;
        size = JSUS.size;
        sample = JSUS.sample;
        isInt = JSUS.isInt;
        ...
        isInt = require('./parse').isInt;
        isArray = require('./array').isArray;
        ({ keys, mixin, size } = require('./obj'));
        ({ sample, randomString, randomInt } = require('./random'));
    }
    ...
    DOM.sprintf = function(string, args, root) {
        ...
        idxs = keys(spans).sort(function(a, b){ return a - b; });
        idx_finish = 0;
    ...
    DOM.blinkTitle = (function(id) {
        ...
        return function(titles, options) {
            ...
            if ('undefined' !== typeof options.repeatFor) {
                nRepeats = isInt(options.repeatFor, 0);
                if (false === nRepeats) {
