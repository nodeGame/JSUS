...
(function(JSUS) {
    ...
    DOM.get = function(name, attributes) {
        var el;
        if (name === 'label') {
            el = DOM.getLabel(arguments[2], arguments[3], attributes);
        }
        else {
    ...
    W.add('label', forEl.parentNode, { 'for': forEl.id }, 'my label', forEl);
    W.add('label', 'my label', forEl.parentNode);

    W.add('span', root, { innerHTML
     ...
     *
     * Special elements with extra parameters
     * 
     *   - 'label': accepts two extra parameter
     *
    ...
    DOM.add = DOM.append = function(name, root, attributes) {
        ...
        if (name === 'label') {
            // TODO: continue here.
            el = DOM.getLabel(arguments[2], arguments[3], attributes);
            if (arguments[3]) root.insertBefore(el, forElem);
     ...
     *   - 'label': adds a element in front (element.parentNode must exist)
     * 
     * @param {HTMLElement} elem The element to decorate
     * @param {object} attributes Object containing attributes to
     *   add to the element
     *
     * @return {HTMLElement} The element with attributes
     *
     * @see DOM.addLabel
     * @see DOM.addClass
     * @see DOM.style
     ...
     *   for the label or its id
     * 
     * @return {HTMLElement} label The label element
     *
     * @see DOM.addAttributes2Elem
     *
