...
(function(JSUS) {
     ...
     * ## OBJ.classClone
     *
     * Creates a perfect copy (keeping class) of the object passed as parameter
     *
     ...
     */
    OBJ.classClone = function(obj) {
        var clone;
        if (obj && 'object' === typeof obj) {
            clone = JSUS.createObj(obj.constructor.prototype);
            for (i in obj) {
                if (obj.hasOwnProperty(i)) {
                    if (obj[i] && 'object' === typeof obj[i]) {
                        clone[i] = JSUS.classClone(obj[i]);
                    }
                    else {
                        clone[i] = obj[i];
                    }
