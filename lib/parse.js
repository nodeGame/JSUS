...
(function(JSUS) {
    ...
    // available can be an array, a string or a object {next(),finished()}
    PARSE.range = function(description, available) {
        var alternatives, i, j;
        ...
        // (n,m),(n,m],[n,m),[n,m] -> (x > n && x < m), ..., (x >= n && x <= m).
        expr = expr.replace(/([(\[]) *([-+]?\d+) *, *([-+]?\d+) *([\])])/g,
        ...
        // a, b -> (a) || (b)
        splitOnComma = expr.spilt(',');
        expr = "(";
        for (i in splitOnComma) {
            if (splitOnComma.hasOwnProperty(i)) {
                expr += splitOnComma[i] + ") || (";
            }
        }
        expr = expr.slice(0, -5);
