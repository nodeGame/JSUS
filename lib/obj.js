...
(function(JSUS) {
     ...
     *   - concatChar: a character to inter between parent and children keys;
     *                 (default: '.')
    ...
    OBJ.keys = (function() {
        return function(obj, level, options) {
            var keys, type, allKeys, leafKeys, levelKeys, concatChar;
            options = options || {};
            ...
            if ('string' === typeof level) level = parseInt(level, 10);

            if (options.concat) {
                if ('undefined' === typeof options.concatChar) concatChar = '.';
                else concatChar = options.concatChar;
            }

            if (!options.concat && options.distinct) keys = {};            

            return _keys(obj, level || 0, 0, options.curParent || '',
                         options.concat, allKeys, leafKeys, levelKeys,
                         concatChar, options.array || [], keys);
        }

        function _keys(obj, level, curLevel, curParent,
                       concatKeys, allKeys, leafKeys, levelKeys,
                       concatChar, res, uniqueKeys) {
                ...
                if (obj.hasOwnProperty(key)) {
                    ...
                    if (isObj && (curLevel < level)) {
                        _keys(obj[key], level, (curLevel+1),
                              concatKeys ? curParent + key + concatChar : key,
                              concatKeys, allKeys, leafKeys, levelKeys,
                              concatChar, res, uniqueKeys);
                    }
                }
